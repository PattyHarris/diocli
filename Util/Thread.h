...
typedef void *LPVOID;
#else
// Diomede: remove these inclusions needed only
// for MFC apps.
// #include "afx.h"
// #include <windows.h>
#include <stdio.h>
...
#if 0
// Diomede: remove - only needed for MFC apps.
class CThread
#ifdef WINDOWS
...
#define STACK_FULL                 0x200
	...
	std::string   m_szObjectCondition;
...
public:
	...
	void		SetPriority(DWORD dwPriority=0);
                ...
                //-----------------------------------------------------
	DWORD		GetErrorFlags() { return m_dwObjectCondition; }
	std::string GetErrorString() { return m_szObjectCondition; }
...
#endif
#else
		thisThreadsId = (ThreadId_t)pthread_self();
#endif
#endif
