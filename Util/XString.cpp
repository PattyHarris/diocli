				...
				TCHAR * lpszResult)
{
	...
	if (nCount > 0)
	{
		...
		assert(pszResultStr[nResultStrSize-2] == _T('\0'));
...
//
std::string _format(const TCHAR *fmt, ...)
{
    std::string retStr("");
    ...
    if (NULL != fmt)
    {
        ...
        #if defined(__x86_64__)
            va_copy(markerRetry, marker);
        #endif

        // Get formatted string length adding one for the NULL
        #ifdef WIN32
            int len = (int)_vscprintf(fmt, marker) + 1;
        #else
            char tmpBuffer[ 1024 ];
            tmpBuffer[ 0 ] = 0;
            int len = vsprintf(tmpBuffer, fmt, marker) + 1;
        #endif
        ...
        #else
            int nWritten = vsnprintf(&buffer[0], len, fmt, markerRetry);
        #endif
...
// _strlwr()
//
// Purpose:     convert the string to lower case
//
