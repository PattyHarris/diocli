...
,m_stackSize(DEFAULT_STACK_SIZE)
{
	...
	{
	    // Diomede: removing to provide output
	    // via caller. 
...
CThread::Push( LPVOID lpv )
{
	...
	if( m_queuePos+1 >= m_chQueue ) {
		...
		m_szObjectCondition = _T("Thread stack overflow.");
		m_mutex.Unlock();
		return FALSE;
	}
...
CThread::Start()
{
	try
	{
		...
		if ( !m_thread ) {
			...
			m_szObjectCondition = _T("Thread creation failed.");
...
CThread::~CThread(void)
{
	if( m_bRunning ) // gracefull termination
	{
		try
		{
			if( !Stop() )
			{
				throw "\n\thread failed to stop in a timely manner!\n";
			}
		}
		catch( char *psz )
		{
