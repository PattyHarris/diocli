...
{
	if( (int)keysize<1 )
		throw CBlowFishException(std::string("Incorrect key length"));
	...
	for(i=0; i<18; i++)
	{
		...
		for(int n=4; n--; )
		{
		    // Diomede: Uncomment below for debugging, otherwise, iVal is
		    // not needed.
			// int iVal = (int)(*p);
			x <<= 8;
			x |= *(p++);
...
void CBlowFish::Encrypt(unsigned char* buf, size_t n, int iMode)
{
	...
	if((n==0)||(n%8!=0))
		throw CBlowFishException("Incorrect buffer length");
	SBlock work;
