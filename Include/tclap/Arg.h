...
class Arg
{
	...
	protected:
		 ...
		 *
		 * Diomede: allows for flags of variable length.  Required
		 * for optional arguments only.
		 * Syntax: /<argument name>=<value>
		 * Example: searchfiles /maxsize=100
		 *
		 * This value (preceded by a dash {-}, Diomede, preceded by a slash {/}),
		 * can be used to identify an argument on the command line.
		 * The _flag can be blank, in fact this is how unlabeled args work.
		 * Unlabeled args must override appropriate functions to get correct handling.
		 * Note that the _flag does NOT include the dash as part of the flag.
		 */
		std::string _flag;

		/**
		 * A single work name indentifying the argument.
		...
		/**
		 * Diomede: Indicates the count of reprompts on this argument.
		 * Generally used for required arguments.
		 */
		int _repromptCount;
	...
	public:
		...
		/**
		 * The char that indicates the beginning of a flag.  Originally '-'.
		 * Diomede: set the flag start char to '/' (Linux, use '-' to
		 *          accomodate path delimiter differences.
		 */
		...
		/**
		 * The string that indicates the beginning of a flag.  Originally "-".
		 * Should be identical to flagStartChar.
		 * \param nOption - The option index identifying one of the possible flag arguments.
		 * Diomede: set the flag start string to '/'.  (Linux, use '-' to
		 *          accomodate path delimiter differences.
		 */
		static const std::string flagStartString(int nOption=0) {
		    #ifdef WIN32
		        std::string szFlagStartString = "/";
		        if (nOption == 1) {
		            szFlagStartString = "-";
		        }
		        else if (nOption == 2) {
		...
		/**
		 * The string that indicates the beginning of a name.  Originally "--".
		 * Should be flagStartChar twice.
		 * Diomede: set the name start string to '//'.  (Linux, use '--' to
		 *          accomodate path delimiter differences.
		 */
		...
		/**
		 * Diomede: Returns the current reprompt count
		 */
		virtual int getRepromptCount() const;

		/**
		 * Diomede: Increments the reprompt count
		 */
		virtual void incrementRepromptCount();
		virtual void resetRepromptCount();
...
#ifdef WIN32
// Diomede: Allow for flag differences between Windows and Linux.  Windows
// allows for either a '/' or a '-' as a flag delimiter.  Linux allows for
// only a '-' due to path delimeter (e.g. /home/user..
...
{
    #if 0
    // Diomede: flags are variable in length, and use the format
    // /<argument name>=<value>
	if ( _flag.length() > 1 )
	...
	if ( _name != ignoreNameString() &&
		 ( _flag == Arg::flagStartString(0) ||
		   _flag == Arg::flagStartString(1) ||
		   _flag == Arg::flagStartString(2) ||
		   _flag == Arg::nameStartString() ||
		   _flag == " " ) )
		throw(SpecificationException("Argument flag cannot be either '" +
							Arg::flagStartString(0) + "' or '" +
							Arg::flagStartString(1) + "' or '" +
							Arg::flagStartString(2) + "' or '" +
							Arg::nameStartString() + "' or a space.",
							toString() ) );

	if ( ( _name.find( Arg::flagStartString(0), 0 ) != std::string::npos ) ||
	     ( _name.find( Arg::flagStartString(1), 0 ) != std::string::npos ) ||
	     ( _name.find( Arg::flagStartString(2), 0 ) != std::string::npos ) ||
...
inline std::string Arg::longID( const std::string& valueId, const bool& bShowName /*false*/ ) const
{
	std::string id = "";

	if ( _flag != "" )
	{
		id += flagStartChar() + _flag;
 ...
 * Diomede: Returns the current reprompt count
 */
inline int Arg::getRepromptCount() const { return _repromptCount; }

/**
 * Diomede: Increments the reprompt count
 */
inline void Arg::incrementRepromptCount()
{
    _repromptCount ++;
}
 ...
 * Diomede: Increments the reprompt count
 */
inline void Arg::resetRepromptCount()
{
    _repromptCount = 0;
}
...
{
    bool bValidArgFlag = ( ( argFlag == Arg::flagStartString(0) + _flag )  ||
                           ( argFlag == Arg::flagStartString(1) + _flag )  ||
                           ( argFlag == Arg::flagStartString(2) + _flag ) );
	if ( ( bValidArgFlag && _flag != "" ) ||
